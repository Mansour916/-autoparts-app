...
export default function GaragePage() {
  ...
  const saveMileage = async (garageId) => {
    try {
      ...
      setVehicles(vehicles.map(v =>
        v.garage_id === garageId ? { ...v, mileage: parseInt(mileageInput) } : v
      ))
      setEditingId(null)
    } catch (err) {
      console.error(err)
  ...
  const fuelConfig = {
    electric: { label: 'Electrique', bg: 'bg-blue-50', text: 'text-blue-700', border: 'border-blue-200' },
      ...
      <div className="bg-ev-dark">
        <div className="max-w-6xl mx-auto px-4 py-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">{vehicles.length} vehicule(s) enregistre(s)</p>
          </div>
          <button
            onClick={() => navigate('/')}
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        {isLoading ? (
          ...
          <div className="bg-white rounded-2xl border border-slate-200 text-center py-20 shadow-sm">
            ...
            <p className="text-slate-600 font-semibold text-lg mb-2">Aucun vehicule dans votre garage</p>
            <p className="text-slate-400 text-sm mb-6">Ajoutez votre premier vehicule en recherchant par VIN ou plaque</p>
            <button
              onClick={() => navigate('/')}
              className="px-8 py-3 bg-ev-blue hover:bg-blue-700 text-white rounded-xl font-bold transition-all"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {vehicles.map(vehicle => {
              const fuel = fuelConfig[vehicle.fuel_type] || {
                ...
                <div key={vehicle.garage_id} className="bg-white rounded-2xl border border-slate-200 hover:border-ev-blue/40 hover:shadow-lg transition-all overflow-hidden">
                  ...
                  <div className="p-5">
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      ...
                      <div className="bg-slate-50 rounded-xl p-3">
                        <p className="text-slate-400 text-xs font-medium mb-1">Kilometrage</p>
                        {editingId === vehicle.garage_id ? (
                          <div className="flex gap-1">
                            ...
                            />
                            <button
                              onClick={() => saveMileage(vehicle.garage_id)}
                              className="bg-ev-green text-white rounded-lg px-2 text-xs font-bold"
                            >
                              OK
                            </button>
                          </div>
                          ...
                          <p
                            onClick={() => {
                              setEditingId(vehicle.garage_id)
                              setMileageInput(vehicle.mileage.toString())
                            }}
